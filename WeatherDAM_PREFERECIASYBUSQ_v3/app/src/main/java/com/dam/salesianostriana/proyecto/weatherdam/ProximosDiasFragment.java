...
public class ProximosDiasFragment extends Fragment {
    ...
    public ProximosDiasFragment(String nombre_ciudad) {
    ...
    private class GetDataTask extends AsyncTask<Void, Void, java.util.List<List>> {
        ...
        protected java.util.List<List> doInBackground(Void... params) {
            ...
            try {
                url = new URL(Utils.URL_BASE_FIVEDAYS()+nombre_ciudad.replace(" ","%20").replace("España","Spain")+Utils.URL_BASE_APPEND());
        ...
        protected void onPostExecute(java.util.List<List> listWeather) {
            ...
            if (listWeather != null) {
                ...
                for (int i = 0; i < listWeather.size(); i = i + 8) {
                    ...
                    String url_image = Utils.URL_BASE_IMG_WEATHER() + listWeather.get(i).getWeather().get(0).getIcon() + Utils.EXTENSION_IMG_WEATHER();
                    ...
                    ciudades.add(new ItemCiudad(url_image, dia_semana,fecha, temp_max, temp_min));
